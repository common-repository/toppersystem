...
class TopperSystemExecuteFileSavePoscssfilests {
...
if(($paginaprovenienzats != "$paginaprovenienzats_controllo")){
    if((isset($_SERVER["REMOTE_ADDR"]))&&(trim($_SERVER["REMOTE_ADDR"]) != "")){
		if((isset($_SERVER["HTTP_USER_AGENT"]))&&(trim($_SERVER["HTTP_USER_AGENT"]) != "")){
	        ...
	        if((isset($_POST["csscriticalinline_toppersystem_send"]) && trim($_POST["csscriticalinline_toppersystem_send"]) != "")) {
		        $csscriticalinline_toppersystem=wp_kses(trim(TopperSystemSafeNOSQL::sanitizeNosql($_POST['csscriticalinline_toppersystem_send'])), array());	
	        }else{
		        $csscriticalinline_toppersystem="";
	        }

	        if((isset($_POST["cssforstyle_toppersystem_send"]) && trim($_POST["cssforstyle_toppersystem_send"]) != "")) {
		        $cssforstyle_toppersystem_send=wp_kses(trim(TopperSystemSafeNOSQL::sanitizeNosql($_POST['cssforstyle_toppersystem_send'])), array());	
	        }else{
		        $cssforstyle_toppersystem_send="";
	        }
	        ...
	        if((trim($canonical_toppersystem_send) != "")&&(trim($resend_toppersystem) != "")&&(trim($hreflang_toppersystem) != "")){
                ...
                if(($authcode_toppersystem_verifica != "")&&($authcode_toppersystem_verifica != "$authcode_toppersystem")){
                    ...
                    if(($authcode_toppersystem_verifica == "$auth_memory")){
                        ...
                        if($csscriticalinline_toppersystem != ""){
                            if($csscriticalinline_toppersystem == "true"){
                                if(file_exists($pathfilenocsscriticalinlinetoppersystem)){
                                    chmod($pathfilenocsscriticalinlinetoppersystem,0644);
                                    @unlink($pathfilenocsscriticalinlinetoppersystem);
                                    $save="1";
                                }
                            }else if($csscriticalinline_toppersystem == "false"){
                                if(!file_exists($pathfilenocsscriticalinlinetoppersystem)){
                                    $textfilem_cssinline_ts=fopen($pathfilenocsscriticalinlinetoppersystem, "w");
                                    fwrite($textfilem_cssinline_ts,"");
                                    fclose($textfilem_cssinline_ts);
                                    unset($textfilem_cssinline_ts);
                                    chmod($pathfilenocsscriticalinlinetoppersystem,0444);
                                    $save="1";
                                }
                            }else{
                                #npt
                        ...
                        if($cssforstyle_toppersystem_send != ""){
                            if($cssforstyle_toppersystem_send == "true"){
                                if(file_exists($pathfilenoactivecssforstyletoppersystem)){
                                    chmod($pathfilenoactivecssforstyletoppersystem,0644);
                                    @unlink($pathfilenoactivecssforstyletoppersystem);
                                    $save="1";
                                }
                            }else if($cssforstyle_toppersystem_send == "false"){
                                if(!file_exists($pathfilenoactivecssforstyletoppersystem)){
                                    $textfilem_cssforstyle_ts=fopen($pathfilenoactivecssforstyletoppersystem, "w");
                                    fwrite($textfilem_cssforstyle_ts,"");
                                    fclose($textfilem_cssforstyle_ts);
                                    unset($textfilem_cssforstyle_ts);
                                    chmod($pathfilenoactivecssforstyletoppersystem,0444);
                                    $save="1";
                                }
                            }else{
                                #npt
...
@add_action('init', TopperSystemExecuteFileSavePoscssfilests::runts(), 2);
}
}
