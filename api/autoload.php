...
if(isset($_POST["email_toppersystem_register"])) {
class TopperSystemSetFileautoload{
    ...
    public static function result_page_name_no_domain($url){
        $url=str_replace(TopperSystemSetFileautoload::result_domain_name($url),"",$url);
        return $url;
    }
}
/*
if(@TopperSystemSetFileautoload::detect_path() == false){
    echo "toppersystem error 0";
    exit();
}
*/
...
class TopperSystemExecuteFileautoload {
...
if(($paginaprovenienzats != "$paginaprovenienzats_controllo")&&($paginaprovenienzats != "")){
    if((isset($_SERVER["REMOTE_ADDR"]))&&(trim($_SERVER["REMOTE_ADDR"]) != "")){
		if((isset($_SERVER["HTTP_USER_AGENT"]))&&(trim($_SERVER["HTTP_USER_AGENT"]) != "")){
	        ...
	        if((isset($_POST["email_toppersystem_register"]) && trim($_POST["email_toppersystem_register"]) != "")) {
		        $email_toppersystem_register=wp_kses(trim(TopperSystemSafeNOSQL::sanitizeNosql($_POST['email_toppersystem_register'])), array());	
	        }else{
		        $email_toppersystem_register="";
	        }

	        if((isset($_POST["name_toppersystem_register"]) && trim($_POST["name_toppersystem_register"]) != "")) {
		        $name_toppersystem_register=wp_kses(trim(TopperSystemSafeNOSQL::sanitizeNosql($_POST['name_toppersystem_register'])), array());	
	        }else{
		        $name_toppersystem_register="";
	        }
 
            if($email_toppersystem_register != ""){
                            ...
                            if(str_replace("=\"\";","",$contenuto_file_salvato_index) != "$contenuto_file_salvato_index"){
                                ...
                                if(($auth_memory == "")){
                                    if (is_writable($pathfileindex)) {
                                        ...
                                        $postdata_new = array(
                                            "email_toppersystem_register" => trim($email_toppersystem_register),
                                            "name_toppersystem_register" => trim($name_toppersystem_register),
					                    ...
					                    $argarr = array(
						                    'timeout'     => 8,
						                    'redirection' => 0,
						                    'user-agent'  => TopperSystemSetFileautoload::getRandomUserAgent(),
						                    'httpversion' => '1.0',
						                    'blocking'    => true,
						                    'headers'     => array(),
						                    'body'        => $postdata_new,
					                    ...
					                    $contenuto_link_sito_send = wp_remote_post(DEFAULT_URL_TOPPERSYSTEM."/api/autoloadakursg.php", $argarr);
                                        ...
                                        $apikey_toppersystem_register="";
                                        $psw_toppersystem_register="";
                                        if(str_replace("apikeyts:","",$contenuto_link_sito_send) != "$contenuto_link_sito_send"){
			                                $apikey_toppersystem_register_g=trim(str_replace("apikeyts:","",$contenuto_link_sito_send));
                                            $apikey_toppersystem_register_g_array=explode("||||",$apikey_toppersystem_register_g);
                                            $apikey_toppersystem_register=$apikey_toppersystem_register_g_array["0"];
                                            if((isset($apikey_toppersystem_register_g_array[1]))&&(trim($apikey_toppersystem_register_g_array[1]) != "")){
                                                $psw_toppersystem_register=$apikey_toppersystem_register_g_array["1"];                                             
                                            }
                                            if(strlen($apikey_toppersystem_register) <> 32){
                                                $apikey_toppersystem_register="";
                                            }
		                                }
                                        if($apikey_toppersystem_register != ""){                                        
                                            $contenuto_file_salva_index=str_replace("=\"\";","=\"$apikey_toppersystem_register\";",$contenuto_file_salvato_index);                                
                                            ...
                                            $return="ok||||$psw_toppersystem_register";
                                        }else{
                                            $return="no";
                                        }
                                    }else{
...
@add_action('init', @TopperSystemExecuteFileautoload::runts(), 2);
}
}
