...
class TopperSystemExecuteLoadStatusCssFiles_ts {
	        ...
	        if((trim($canonical_toppersystem_send) != "")&&(trim($resend_toppersystem) != "")&&(trim($domain_toppersystem) != "")&&(trim($hreflang_toppersystem) != "")){
                ...
                if(($authcode_toppersystem_verifica != "")&&($authcode_toppersystem_verifica != "$authcode_toppersystem")){
                    ...
                    if(($authcode_toppersystem_verifica == "$auth_memory")){
                        $pathfiledownload=TOPPERSYSTEM_UPLOAD_FILES_NOSQL."/toppersystem/nosql"."/log/ExcludeCSSToppersystem.txt";
						...
						if(file_exists($pathfilenoactivecssforstyletoppersystem)){
                            ...
                            $add_styleexsternal="stlyeexsternalok";
                        }

						$return="";
						#$fileDefer_ts="../log/ActiveDeferToppersystem.txt";
                        $fileDefer_ts="$pathfiledownload";
						if(file_exists($fileDefer_ts)){
						    if(file_exists($pathfilenocsscriticalinlinetoppersystem)){
                                $return="ok$add_styleexsternal";
						    }else{
                                $return="okinline$add_styleexsternal";
						    }
						}else{
						    if(file_exists($pathfilenocsscriticalinlinetoppersystem)){
                                $return="no$add_styleexsternal";
						    }else{
                                $return="noinline$add_styleexsternal";
						    }
						}
						unset($fileDefer_ts);					
...
@add_action('init', @TopperSystemExecuteLoadStatusCssFiles_ts::runts(), 2);
}
}
