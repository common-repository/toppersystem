...
class TopperSystemSetFilestatusOptimization {
    ...
    public static function result_page_name_no_domain($url){
        $url=str_replace(TopperSystemSetFilestatusOptimization::result_domain_name($url),"",$url);
        return $url;
    }
}

class TopperSystemExecuteFilestatusOptimization {
                ...
                if(($authcode_toppersystem_verifica != "")&&($authcode_toppersystem_verifica != "$authcode_toppersystem")){
                    ...
                    if(($authcode_toppersystem_verifica == "$auth_memory")){         
                            ...
                            if(file_exists($pathfileindex)){
                                ...
                                $canonical_toppersystem_send_no_domain=TopperSystemSetFilestatusOptimization::result_page_name_no_domain("$canonical_toppersystem_send");
                                ...
                                if($line_preleva != ""){ 
                                    if (isset(explode("||||",$line_preleva)["1"])){                                   
                                        ...
                                        if(trim("$file_name") != ""){
                                            if(file_exists($pathdirdb.$file_name)){
                                                if(date("U") - filemtime($pathdirdb.$file_name) <= 480){                                  
                                                    $contenuto_link_sito_send="ok";
                                                }else{                                   
                                                    $contenuto_link_sito_send=date("U") - filemtime($pathdirdb.$file_name);                                               
                                                }
                                            }else{                                   
                                                $contenuto_link_sito_send=date("U");                                               
                                            }
                                        }
                                    }
                                }
...
@add_action('init', @TopperSystemExecuteFilestatusOptimization::runts(), 2);
}
}
